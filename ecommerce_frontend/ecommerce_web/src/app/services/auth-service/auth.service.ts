  ...
  login(username: string, password: string): any {
    ...
    .pipe(
      ...
      map((res: HttpResponse<any>) => {
        ...
        const tokenLength = res.headers?.get(AUTH_HEADER)?.length; //had to add ? and ? if not I got an error Object could be null
        const bearerToken = res.headers?.get(AUTH_HEADER)?.substring(7, tokenLength); //had to add ? and ? if not I got an error Object could be null
